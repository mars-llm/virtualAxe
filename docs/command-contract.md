...
| `./vaxe --source bitaxe --status` | tracked-source read-only | Inspects tracked source metadata, the reusable image manifest, existing runtime markers, local container state, and the loopback AxeOS API. It does not fetch, patch, build, start, stop, or rewrite local state. Add `--json` for a stable machine-readable report; use `--source nerdnos` to inspect NerdNos. |
